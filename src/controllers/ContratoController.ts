...
router.get("/", async (req, res) => {
  try {
    let contratos = await contratoService.buscarTodos(
      req.query.offset,
      req.query.limit,
      req.query.order
    );
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.send(
      new ResponseBuilder(true, "Contrato encontrado com sucesso", contrato)
    );
  } catch (error) {
    res.send(new ResponseBuilder(false, error.message));
  }
...
router.get("/cliente/:id", async (req, res) => {
  try {
    let contrato = await contratoService.buscarPorCliente(
      req.params.id,
      req.query.offset,
      req.query.limit,
      req.query.order
...
router.put("/:id", async (req, res) => {
  try {
    let contrato = await contratoService.atualizarContrato(
      req.params.id,
      req.body
    );
    res.send(
      new ResponseBuilder(true, "Contrato atualizado com sucesso", contrato)
    );
  } catch (error) {
    res.send(new ResponseBuilder(false, error.message));
  }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.send(
      new ResponseBuilder(true, "Contrato deletado com sucesso", contrato)
    );
  } catch (error) {
    res.send(new ResponseBuilder(false, error.message));
  }
