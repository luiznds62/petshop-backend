...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.send(
      new ResponseBuilder(
        true,
        "Tipo de contrato encontrado com sucesso",
        tipoContrato
      )
    );
  } catch (error) {
...
router.post("/", async (req, res) => {
  try {
    ...
    res.send(
      new ResponseBuilder(
        true,
        "Tipo de contrato salvo com sucesso",
        tipoContrato
      )
    );
  } catch (error) {
...
router.put("/:id", async (req, res) => {
  try {
    let tipoContrato = await tipoContratoService.atualizarTipoContrato(
      req.params.id,
      req.body
    );
    res.send(
...
router.delete("/:id", async (req, res) => {
  try {
    let tipoContrato = await tipoContratoService.deletarTipoContrato(
      req.params.id
    );
    res.send(
      new ResponseBuilder(
